...
const { data: myDetailsData, isLoading: isLoading } = useMyDetailsQuery
<any>
();
useEffect(() => {
if (myDetailsData) {
            ...
            <div className="h-[28rem] xl:w-[30%]  w-auto rounded-lg bg-[#FEE7BD]  md:p-5 md:h-[26rem]">
               ...
               */}
               ...
               <div className="mt-2 text-center text-lg text-[#007EAF] underline">
                  <Link to={"/questions"}>
                  Click to complete profile</Link>
               </div>
               */}
            </div>
         </div>
...
{/* Partner Preferences */}
<div className="h-auto rounded-xl mb-10 overflow-hidden">
...
<div className="h-auto rounded-xl mt-10">
   <div className="flex flex-col rounded-xl bg-white border border-solid border-gray-200 bg-white pb-6 shadow-sm">
      <div
      className={`justify-center border-b border-solid border-zinc-300 px-6 py-4 text-lg leading-6 tracking-wide ${isExclusive ? 'text-[#007eaf]' : 'text-[#007EAF]'} max-md:px-5 md:text-xl`}
      style={{ fontFamily: "Proxima-Nova-Bold, sans-serif" }}
...
<MyConnectionsModal isVisible={isConnectionsModalVisible} onClose={() =>
setIsConnectionsModalVisible(false)} />
<SentRequestsModal isVisible={isSentRequestsModalVisible} onClose={() =>

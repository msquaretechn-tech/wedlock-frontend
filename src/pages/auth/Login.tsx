    ...
    try {
      ...
      if (successData.success === true) {
        const isLocationFormFilled =
          successData?.user?.isLocationFormFilled;

        const isPersonalFormFilled =
          successData?.user?.isPersonalFormFilled;

        const isImageFormFilled =
          successData?.user?.isImageFormFilled;

        const isQualificationFormFilled =
          successData?.user?.isQualificationFormFilled;

        const isOtherFormFilled =
          successData?.user?.isOtherFormFilled;
        ...
        const suspensionResult =
          await checkSuspendStatus({ userId }).unwrap();

        if (suspensionResult?.isSuspended) {
          navigate("/suspended", {
            state: { suspension: suspensionResult },
          });
          return;
        }
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-md px-2 py-2 mt-2">
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="rounder-[8px]">
            ...
            <button
             ...
             className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700 mt-[5px]"
            >
        ...
        <button
          className="bg-transparent border text-[#ffffff] mt-2 w-full h-12 rounded-md"
